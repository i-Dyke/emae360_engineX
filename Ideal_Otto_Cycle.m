...
plot(the, mfr_in(the))
a_mfr_in = mfr_in(the);
mass_in = trapz((the).*rpm.*360./60, a_mfr_in)
if in_choked
    title("Intake Mass Flow Rate v.s. Crank Angle (Choked)")
else
...
plot(the, mfr_ex(the))
a_mfr_ex = mfr_in(the);
mass_ex = trapz((the).*rpm.*360./60, a_mfr_ex)
if in_choked
    title("Exhaust Mass Flow Rate v.s. Crank Angle (Choked)")
else
